...
func (s *TaskService) GetByID(ctx context.Context, token, id string) (Task, error) {
	_, err := s.authClient.VerifyToken(ctx, token)
	if err != nil {
		return Task{}, fmt.Errorf("auth failed: %w", err)
	}
...
func (s *TaskService) Update(ctx context.Context, token, id string, title *string, done *bool) (Task, error) {
	_, err := s.authClient.VerifyToken(ctx, token)
	if err != nil {
		return Task{}, fmt.Errorf("auth failed: %w", err)
	}
